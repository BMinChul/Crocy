...
const ACTION_KEY_MAPPING: KeyMapping = {
  punch: ['KeyF'],
  kick: ['KeyG', 'Mouse2'],
  meleeAttack: ['Space'], // Space for Attack (Mouse0 removed for Target Selection)
  cast: ['KeyE', 'Mouse1'],
};
...
export const InputController: React.FC<InputControllerProps> = ({ disabled = false, disableKeyboard = false, disableJoystick = false }) => {
  ...
  const toggleStatWindow = useGameStore((state) => state.toggleStatWindow);
  const toggleInventory = useInventoryStore((state) => state.toggleInventory); // Assuming Inventory store handles I key separately, but check below
  // Actually InputController doesn't import InventoryStore yet, maybe I should add it later if needed.
  // For now just StatWindow.
